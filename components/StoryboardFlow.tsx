...
const VersionDisplay = () => {
    const [timeAgo, setTimeAgo] = useState(
        formatDistanceToNow(new Date(__BUILD_DATE__), { addSuffix: true })
    );

    useEffect(() => {
        const interval = setInterval(() => {
            setTimeAgo(formatDistanceToNow(new Date(__BUILD_DATE__), { addSuffix: true }));
    ...
    return (
        <span className="text-zinc-500 font-medium">
            v{__APP_VERSION__} • Updated {timeAgo}
        </span>
    );
};
...
const FlowEditor = () => {
    ...
    const [enableNanoBanana, setEnableNanoBanana] = useState(false); // Default OFF
    ...
    const [appSettings, setAppSettings] = useState<AppSettings>(() => {
        ...
        try {
            ...
            if (oldStored && oldStored !== 'undefined') {
                ...
                return {
                    provider: oldParsed.provider || Provider.CUSTOM,
    ...
    }, [setNodes, setEdges, handleRunVariation, appSettings]);
        ...
        <div className="w-full h-screen bg-background relative">
            ...
            <div className="absolute top-6 left-6 pointer-events-none">
                ...
                <p className="text-zinc-500 text-sm mt-1 flex items-center gap-2">
                    <span>Node-based Generative AI Workflow</span>
                                ...
                                <div className="flex items-center gap-2 mb-3">
                                    <label className="relative inline-flex items-center cursor-pointer">
                                        <input
                                            ...
                                            onChange={(e) => {
                                                setEnableNanoBanana(e.target.checked);
                                                if (!e.target.checked && appSettings.provider === Provider.GOOGLE) {
                                                    setAppSettings(prev => ({ ...prev, provider: Provider.CUSTOM }));
                                                }
                                            }}
                                        />
                                        <div className="w-9 h-5 bg-zinc-700 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-yellow-500"></div>
                                        <span className="ml-2 text-xs text-zinc-500 font-medium">Enable Nano Banana (Experimental)</span>
                                    </label>
                                </div>

                                <div className="grid grid-cols-2 gap-2">
                                    {[Provider.CUSTOM, ...(enableNanoBanana ? [Provider.GOOGLE] : [])].map(p => (
                                        <button
                                            key={p}
                                            onClick={() => setAppSettings(prev => {
                                                ...
                                                if (p === Provider.CUSTOM) {
                                                    newImgModel = 'google/gemini-2.5-flash-image';
                                            ...
                                            className={`py-2 px-3 rounded-lg text-xs font-bold uppercase transition-all border ${appSettings.provider === p
                                        ...
                                        <div>
                                            ...
                                            <div className="relative">
                                                <select
                                                    ...
                                                    className="w-full bg-zinc-950 border border-border rounded-lg p-2.5 text-sm text-white focus:border-accent focus:ring-1 focus:ring-accent outline-none appearance-none pr-8"
                                                >
                                                    ...
                                                    <option value="flux/flux-schnell">flux/flux-schnell</option>
