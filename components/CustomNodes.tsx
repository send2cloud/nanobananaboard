...
export const StartNode: React.FC<NodeProps<StartNodeData>> = ({ data, selected }) => {
  ...
  // Configuration State
  const [model, setModel] = useState(isGoogle ? MODEL_OPTIONS.FLASH : (data.globalModel || 'google/gemini-2.5-flash-image'));

  // Sync model when provider/global settings change
  useEffect(() => {
    if (isGoogle) {
      if (!Object.values(MODEL_OPTIONS).includes(model as any)) {
        setModel(MODEL_OPTIONS.FLASH);
      }
    } else if (data.globalModel) {
      setModel(data.globalModel);
    }
  }, [isGoogle, data.globalModel]);
  ...
  const handleGenerate = () => {
    ...
    const config: GenerationConfig = {
      model: model, // Use local state (synced with global or overridden)
      aspectRatio,
      style,
      shotType,
      cameraAngle,
                  ...
                  {isGoogle ? (
                    ...
                    <div className="relative">
                      <select
                        value={model}
                        onChange={(e) => setModel(e.target.value)}
                        className="w-full bg-zinc-950 border border-border rounded px-2 py-1.5 text-xs text-zinc-300 focus:outline-none focus:border-zinc-600 appearance-none pr-6"
                      >
                        ...
                        <option value="flux/flux-schnell">Flux Schnell</option>
                  ...
                  <div className="grid grid-cols-5 gap-1">
                    {ASPECT_RATIOS.map(ar => (
                      <button
                        ...
                        className={`py-1.5 text-[10px] rounded border transition-all ${aspectRatio === ar.value
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {STYLES.map(s => (
                      <button
                        ...
                        className={`px-2 py-1 text-[10px] rounded-full border transition-all ${style === s.value
...
export const VariationNode: React.FC<NodeProps<VariationNodeData>> = ({ id, data, selected }) => {
  ...
  // Determine if using Google provider (default or explicit)
  const isGoogle = !data.provider || data.provider === Provider.GOOGLE;
  ...
  const handleRun = () => {
    ...
    data.onGenerate(id, {
      ...
      model: isGoogle ? model : (data.globalModel || 'google/gemini-2.5-flash-image')
    });
  };
  ...
  return (
    <NodeWrapper selected={selected} title="Configuration" width="w-[380px]">
      ...
      <div className="grid grid-cols-4 gap-1 mb-4">
        {CATEGORIES.map((cat) => (
          <button
            ...
            className={`flex flex-col items-center justify-center p-2 rounded-lg transition-all ${category === cat.id
      ...
      {/* Model Selection (Hidden if Google/Default, shown if advanced needed? Actually restoring simple view first, or adding dropdown? user wants dropdown. I'll add the dropdown back) */}
      <div className="mb-3 px-1">
        {isGoogle ? (
          <div className="flex items-center gap-2 text-[10px] text-zinc-500">
            <div className="w-2 h-2 rounded-full bg-green-500"></div>
            <span>Using: {(model === MODEL_OPTIONS.FLASH) ? 'Gemini Flash' : 'Gemini Pro'}</span>
          </div>
        ) : (
          <select
            value={model}
            onChange={(e) => setModel(e.target.value)}
            className="w-full bg-zinc-950/50 border border-border rounded p-1.5 text-xs text-zinc-400"
          >
            <option value="google/gemini-2.5-flash-image">Gemini Flash</option>
            <option value="google/gemini-3-pro-image-preview">Gemini Pro</option>
            <option value={data.globalModel || 'custom'}>Inherit Global</option>
          </select>
        )}
      </div>
